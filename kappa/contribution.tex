...
To evaluate \textsc{IntraJ}, we compared it with \textsc{SonarQube}~\cite{sonarqube}, an industrial 
static analyzer. Additionally, we compared the precision and performance of 
...
%\begin{figure}[H]
%  \includegraphics[width=1\linewidth]{kappa/img/DDAAnalysisTime.pdf}
%  \caption{\label{fig:DAAAnalysisOnly}
%    Time for executing only the \code{DeadAssignment} Analysis.}
%\end{figure}

The results of the evaluation for \code{DeadAssignment} analysis are shown in Figure~\ref{fig:DAATotalTime}.
...
The results of the evaluation for \code{NullPointerException} analysis are shown in Figure~\ref{fig:NPETotal} and Figure~\ref{fig:NPEOnly}.
As for the \code{DeadAssignment} analysis, we compared both the total time to process
each benchmark, and the total time to load and run the analysis.
As can be seen, \textsc{IntraJ} is faster than \textsc{SonarQube} in all cases. 
Even by excluding the \textcolor{grey}{\emph{Baseline}}, \textsc{IntraJ} is still faster than \textsc{SonarQube} for all benchmarks.
A strict comparison to SQ is therefore difficult,
but we observe that INTRAJ is considerably faster including
the baseline, at most 3.12 times slower for DAA only, and
considerably faster for NPA only, though the latter is likely
due to SQ's more expensive interprocedural analysis.
