...
We evaluated the performance and precision of \textsc{IntraJ} on a set of 4 well-knwon 
open-source Java projects.
The projects are: the \textsc{ANTLR} parser generator, the \textsc{PMD} static code analyzer,
...
We compared \textsc{IntraJ} with \textsc{SonarQube} on two different analyses: \code{DeadAssignment} 
...
The results of the evaluation for \code{DeadAssignment} analysis are shown in Figure~\ref{fig:DAATotalTime} and Figure~\ref{fig:DAAAnalysisOnly}.
In Figure~\ref{fig:DAATotalTime}, we report two different measurements: the total 
...
In Figure~\ref{fig:DAAAnalysisOnly}, we compare only the time for executing the analysis.
...
\begin{figure}[H]
  \includegraphics[width=1\linewidth]{kappa/img/NPAAnalysisTime.pdf}
  \caption{\label{fig:NPEOnly}Time for executing only the \code{NullPointerException}.}
\end{figure}
...
As for the \code{DeadAssignment} analysis, we compare both the total time to process
