...
The overall architecture of our proposed framework, \textsc{IntraCFG}, is shown in Figure~\ref{fig:intraCFG}.
The framework provides the skeleton and default behaviour for constructing CFGs,
...
(e.g., \astnode{CFGRoot}, \astnode{CFGSupport}, and \astnode{CFGNode}), attribute equations that define the
...
The framework exposes the CFG to client analyses through APIs, which can query 
the CFG for information, such as the entry and exit nodes, or traverse the CFG.
...
by selectively excluding/including specific nodes from the CFG.
...
On the other hand, the precision of the CFGs can be improved by synthesising new nodes and subtrees.
...
define for each AST node in the CFG to capture the semantics of passing
through that node.
...
define their types.
For simplicity, we will use TEAL-0 to exemplify the use of RAGs
...
construct enhanced control-sensitive CFGs, i.e., it understands that \texttt{if (x != null)\{x.f=1\}}, is safe. Control-sensitive CFGs
...
synthesizing two AST nodes, i.e., \astnode{ControlTrue} and \astnode{ControlFalse} for each
comparison operator, e.g.,  ``$\le$'', ``$==$'', ``$!=$'', inside a conditional expression.
These HOAs are used to represent the control flow of the program and the information that can be inferred from
it. \\
%
...
if(x > 4 and x <= 6){
    ...
    \end{lstlisting}
\end{minipage}\\
The example in Listing~\ref{lst:control-null} shows how the \astnode{ControlTrue} and \astnode{ControlFalse}
...
Listing~\ref{lst:control-interval} shows how the \astnode{ControlTrue} and \astnode{ControlFalse}
