...
\begin{figure}[H]
    ...
    \caption{\label{fig:IntraCFGExample} Example of definition of  \Ainhno{CFGNode}{nextNodes} and \Asynno{CFGNode}{firstNodes} for the \textsc{Teal} \astnode{AddExpr} using right-to-left operand evaluation.
    All the nodes are \astnode{CFGNode}s. Dashed arrows indicates the location of the equation for an inherited attribute.
    }
\end{figure}
The example in Figure~\ref{fig:IntraCFGExample} shows how the \Ainhno{CFGNode}{nextNodes} and
\Asynno{CFGNode}{firstNodes} attributes can be defined to achieve the desired CFG.
This example assumes that operands are evaluated right-to-left in order to illustrate the versatility of the approach.
The \astnode{AddExpr} and its operands all implement the \astnode{CFGNode} interface.
For any \astnode{AddExpr}, the \Asynno{CFGNode}{firstNodes} attribute is defined to
be equal to the \Asynno{CFGNode}{firstNodes} of its right operand. The \astnode{AddExpr} node
also defines the \Ainhno{CFGNode}{nextNodes} attribute of both its operands:
for the right operand, it is equal to the \Asynno{CFGNode}{firstNodes} of the left operand,
while for the left operand, it is equal to a singleton set containing the \astnode{AddExpr} node itself.
The \Asynno{CFGNode}{succ} attribute is not overridden by any node, and
therefore, it is defined to be equal to the node's \Ainhno{CFGNode}{nextNodes} attribute.
...
defined by the nearest \astnode{CFGRoot} ancestor.
...
\emph{out}\footnote{Or \emph{kill} and \emph{gen}.} circular attributes for each \astnode{CFGNode} (e.g., \AcircSyn{CFGNode}{in} and  \AcircSyn{CFGNode}{out}).
...
with \textsc{SonarQube} on two distinct analyses: the \code{DeadAssignment} 
and \code{NullPointerException} analysis.

\begin{figure}[H]
  \includegraphics[width=1\linewidth]{kappa/img/DAATotalTime.pdf}
  \caption{\label{fig:DAATotalTime}Total time for the \code{DeadAssignment} Analysis. This includes time for parsing,
  checking the absence of errors, running the analysis.}
\end{figure}


Figure~\ref{fig:DAATotalTime} shows the evaluation results for the \code{DeadAssignment} analysis. 
...
Our analysis shows that \textsc{IntraJ} outperforms \textsc{SonarQube} in all 
...
that \textsc{SonarQube}'s \code{DeadAssignment} analysis is faster than \textsc{IntraJ}'s.
We believe that this is due to the fact that \textsc{SonarQube} computes the 
CFGs during the \textcolor{grey}{\emph{Baseline}} phase.


\begin{figure}[H]
  \includegraphics[width=1\linewidth]{kappa/img/NPETotalTime.pdf}
  \caption{\label{fig:NPETotal}Total time for the \code{NullPointerException}. This includes time for parsing,
  checking the absence of errors, running the analysis.}
\end{figure}


The evaluation results for the \code{NullPointerException} analysis are presented in 
Figure~\ref{fig:NPETotal}. Similarly to the \code{DeadAssignment} analysis, we
...
It can be noticed that, even when excluding the \textcolor{grey}{\emph{Baseline}} measurements, \textsc{IntraJ} 
still achieves faster execution times than \textsc{SonarQube} for all benchmarks.

We compared the precision of \textsc{IntraJ} with \textsc{SonarQube} for the
\code{DeadAssignment} and \code{NullPointerException} analyses. 
...
\begin{figure}[H]
  ...
  \caption{\label{fig:DAAPrecisino}Precision of the \code{DeadAssignment} Analysis.
  We report the ratio of true positives and false positives to the total number of reports.}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=1\linewidth]{kappa/img/NPAPrecision.pdf}
  \caption{\label{fig:NPAPrecision}Precision of the \code{NullPointerException} Analysis.
  We report the ratio of true positives and false positives to the total number of reports.}
\end{figure}

Figure~\ref{fig:DAAPrecisino} shows the results for the \code{DeadAssignment} analysis.
It is evident that \textsc{IntraJ} outperforms 
...
The results for \code{NullPointerException} analysis are shown in Figure~\ref{fig:NPAPrecision}.
For this analysis \textsc{IntraJ} is slightly less precise than \textsc{SonarQube}.
...
\section{\textsc{IntraTeal}: \textsc{IntraCFG} for \textsc{Teal}}
\label{sec:intraTeal}
...
we developed \textsc{IntraTeal}\footnote{
  The complete source code of \textsc{IntraTeal} is available at 
  \href{https://doi.org/10.5281/zenodo.7649171}{10.5281/zenodo.7649171}.
}, an implementation of \textsc{IntraCFG} for the \textsc{Teal} programming language.
% \subsection{The \textsc{Teal} Programming Language}
% \label{sec:teal}
...
% One of the key features of \textsc{Teal} is that it is an object-oriented language,
...
\textsc{IntraTeal} is our implementation of \textsc{IntraCFG} for the \textsc{Teal} language.
...
% The \textsc{Teal} programming language is used for teaching purposes
...
\begin{lstlisting}[language=JastAdd,label={lst:ControlTrueEq},float=h,captionpos=b, caption={Transfer function for \astnode{ControlTrue} HOA.}]
...
\subsection{LSP support via MagpieBridge: warnings, quick-fixes and bug explanations}
\begin{figure}
  \centering
...
\begin{figure}[ht]
  ...
  1. The \texttt{NullPointerException} is detected by \textsc{IntraJ} (squiggly line under \code{x}) with a quick-fix available (\includegraphics[height=8pt]{kappa/img/bulb.png}).
...
\subsection{Visualisation via \textsc{CodeProber}}
