...
be either a  variables, a numeric constants or the \code{null} literal. We also assume
...
Consider the scenario where \code{x} is used dereferenced immediately after the
if-else statement. The program may crash if \code{x} is \code{null}. This is a potential
bug in the program.

This information can be used to identify potential bugs in the program, such as,
unexpected \code{NullPointerException} exceptions.
...
The join operator $\sqcup$ merges the information of two nodes by taking their
...
$V$ be the set of nodes in the CFG, and  $\textit{Var} \rightarrow \mathcal{L}$
...
\end{align*}
where $\text{pred}(n)$ is the set of predecessor of the node $n$, i.e., $\text{pred}(n) = \{ p \mid (p,n)\in E\}$ where $E$ is the set of edges in the CFG.
...
present several techniques for computing fixpoints and demonstrate that exists a fixpoint~\cite{Knaster1929} and that the resulting
fixpoint is unique and the minimum among all fixed points of the transfer function $f$.
...
\astnode{A}$_1$ and \astnode{A}$_2$. If all subtypes will use the same equation, the declaration and the equation can be combined into a single line, as follows:
\begin{equation*}
  \Asyn{A}{x} = e
\end{equation*}
...
\begin{align*}
    ...
    &\Ainhdef{C}{B}{w}  = 5
\end{align*}
resulting in \Ainh{C}{y} = 37 + 5 = 42.

The attribute value of node \astnode{B} depends on the context in which it is
evaluated, resulting in potentially different values. Specifically, the attribute
value of \Ainh{B}{w}  can be either 36 or 5, depending on the context of node \astnode{B}.
In this case, the value of \Ainh{B}{w}
