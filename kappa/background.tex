...
underlies the main results of this thesis. Specifically, in this section we will delve into
...
\begin{tikzpicture}[
    node distance=2.5cm,
    concept/.style={rectangle,draw=black,text width=2cm,text centered},
    ...
    contribution/.style={rectangle,draw=black,text width=2cm,text centered, dashed, fill=blue!10},
    staticanalysis/.style={rectangle,draw=black,text width=2cm,text centered, dashed, fill=red!10},
    ag/.style={rectangle,draw=black,text width=2cm,text centered, dashed, fill=green!10}
    ]
    \node[concept] (dataflow) {\phantom{Dataflow} \phantom{Analysis}};
    \node[concept, below of = dataflow] (cfa) {\phantom{Control-flow} \phantom{Analysis}};
    \node[concept, right of = dataflow] (intracfg) {\phantom{\textsc{IntraCFG}}};
    \node[concept, below of = intracfg] (teal) {TEAL};
	\node[concept, right of = teal] (jastadd) {JastAdd};
    \node[concept, below of = jastadd] (rags) {Reference Attribute Grammars (RAGs)};
	\node[concept, below of = rags] (ags) {Attribute Grammars};
    \node[concept, above of = intracfg] (intrateal) {\phantom{\textsc{IntraTeal}}};
    \draw[contribution] ($(intrateal.north west)+(-0.10,0.10)$) rectangle ($(intracfg.south east)+(0.10,-0.10)$);
    \node[concept, right of = dataflow] (intracfg) {\textsc{IntraCFG}};
    \node[concept, above of = intracfg] (intrateal) {\textsc{IntraTeal}};
    \draw[staticanalysis] ($(dataflow.north west)+(-0.10,0.10)$) rectangle ($(cfa.south east)+(0.10,-0.10)$);
    \node[concept] (dataflow) {Dataflow Analysis};
    \node[concept, below of = dataflow] (cfa) {Control-flow Analysis};
    \draw[ag] ($(jastadd.north west)+(-0.10,0.10)$) rectangle ($(ags.south east)+(0.10,-0.10)$);
    \node[concept, right of = teal] (jastadd) {\textsc{JastAdd}};
    \node[concept, below of = jastadd] (rags) {Reference Attribute Grammars (RAGs)};
	\node[concept, below of = rags] (ags) {Attribute Grammars};
    \path[->,-latex] (dataflow) edge (cfa);
    \path[->,-latex] (jastadd) edge (rags);
    \path[->,-latex] (rags) edge (ags);
    \path[->,-latex] (intracfg) edge (cfa);
    \path[->,-latex] (intracfg) edge (teal);
    \path[->,-latex] (intracfg) edge (jastadd);
    \path[->,-latex] (intrateal) edge (dataflow);
    \path[->,-latex] (intrateal) edge (intracfg);
    \matrix [draw, above left = -000pt and 150pt,inner sep=1ex,cells={nodes={font=\sffamily,anchor=west}},ampersand replacement=\&] at (ags) {
        \node [concept, text width=0.1cm] {}; \& \node{Topic}; \\
        \node [contribution, text width=0.1cm] {}; \& \node{New contribution}; \\
        \node [staticanalysis,text width=0.1cm] {};  \& \node{Static Analysis};\\
        \node [ag,text width=0.1cm] {}; \& \node{Attribute Grammars}; \\
        \draw[-latex,color=black](0,0) -- ++ (0.4,0); \& \node{Depends on relation};\\
      };
  \end{tikzpicture}}
...
of programs by tracking variable defintions and uses. This information can be used to optimize the program by eliminating
...
Monotone framework~\cite{kam1977monotone} is a teoretical framework for reasoning
...
\end{itemize}
In \textsc{JastAdd}, synthesized attributs are defined using the \code{syn} keyword followed
...
The attributes are defined in a ``\code{.jrag}'' file with the following syntax:
    \begin{lstlisting}[language=JastAdd]
        ...
        eq A.C.y() = A.z() + C.v();
    \end{lstlisting}
The last line is the equation that defines the value of the inherited attribute \Ainh{C}{y} for each child of type \astnode{C}.
\begin{figure}
    \begin{center}
        \begin{tikzpicture}
...
syn nta Exit FunDecl.exit() = new Exit();
    ...
    or indirectly on itself. In \textsc{JastaAdd}, circular attributes are expressed using the \code{circular}
    ...
    result of a collection attribute is the aggreagation of contributions that can
    come frome anywhere in the AST. A contribution clause is associated with
...
Expr contributes this when type.compatible(expectedType()) to Program.errors();
    \end{lstlisting}
    The collection attribute, \texttt{Program.errors()}, that we denote with \Acoll{Program}{errors},
