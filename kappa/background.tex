...
\code{NullPointerException} exceptions.
...
information is updated to \code{null}. If the assignment is \code{x = y}, then the
information is updated to the value \code{y} maps to.
\item If $n$ is a conditional, an Entry or an Exit node, nothing is updated.
\end{itemize}
...
In our example, the lattice $L$ is the set of possible values of the variable \code{x}. 
...
The join operator $\bigsqcup$ is the union operator, which is used to combine the
information of two nodes. For example, if the information at node $n$ is \code{null}
and the information at node $n'$ is \code{notnull}, then the information at node
$n \bigsqcup n'$ is \code{maybenull}.
In Section~\ref{sec:dataflowanalysis}, we described the effect of passing through 
a node using natural language. Now, we will formally present this update as our
transfer function. Our monotonic transfer function $f_{\textit{NULL}}: (\textit{Var} \rightarrow L) \times V \rightarrow (\textit{Var} \rightarrow L)$ is defined as:
...
\begin{align*}
  \text{in}(n) &= \begin{cases} \{ v \rightarrow \bot \mid \forall v \in $Var$ \} & $if $ n $ is Entry$ \\ \bigsqcup\limits_{p \in \text{pred}(n)} \text{out}(p) & $otherwise$ \end{cases} \\
  \text{out}(n) &= f_{\mathit{NULL}}(\text{in}(n),n)
\end{align*}
where $\text{pred}(n) = \{ p \mid (p,n)\in E\}$.

To model the information that is available before and after passing through a 
node, and to reflect the impact of passing through the node, we have defined 
two sets: the $in$ set and the $out$ set. The $in$ set gathers the available 
information before the node is passed, and the $out$ set captures the outcome 
of applying the transfer function $f_{\textit{NULL}}$ on the $in$ set.

These equations can be adapted to perform backward analyses as well. For example,
given a monotonic transfer function $f$ and a finite lattice $L$, the equations for 
a backward analysis would be:
