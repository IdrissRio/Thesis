...
%
%\begin{lstlisting}[language=JastAdd, caption=SB module specification., label=listing:SBModule, captionpos=b,float=t]
%MethodDecl implements CFGRoot;
%ExprStmt, IfStmt, AndOp, Block, AssignStmt implements CFGSupport;
%VarAccess, EQOp, IntegerLiteral  implements CFGNode;
%
%eq ExprStmt.firstNodes() = getExpr().firstNodes();
%
%eq EQOp.firstNodes() = getRight().firstNodes();
%eq EQOp.getRight().nextNodes() = getLeft().firstNodes();
%eq EQOp.getLeft().nextNodes() = Collections.singleton(this);
%
%eq AssignExpr.firstNodes() =  getDest().firstNodes();
%eq AssignExpr.getDest().nextNodes() = getSource().firstNodes();
%eq AssignExpr.getSource().nextNodes() = Collections.singleton(this);
%eq AssignExpr.getChild().nextNodesTT() = Collections.singleton(this);
%eq AssignExpr.getChild().nextNodesFF() = Collections.singleton(this);
%
%eq MethodDecl.Entry().nextNodes() =
% (hasBlock() ? getBlock().firstNodes() : Collections.singleton(Exit()));
%eq MethodDecl.getBlock().nextNodes() = Collections.singleton(Exit());
%
%eq Block.firstNodes() =
%   (getNumStmt() == 0) ? nextNodes() : getStmt(0).firstNodes();
%eq Block.getStmt(int index).nextNodes() =
%    (getNumStmt() - 1 == index) ? nextNodes() : getStmt(index + 1).firstNodes();
%
%eq AndOp.firstNodes() = getLeftOperand().firstNodes();
%eq AndOp.getLeftOperand().nextNodesTT() = getRightOperand().firstNodes();
%eq AndOp.getLeftOperand().nextNodesFF() = nextNodesFF();
%eq AndOp.getChild().nextNodes(){
%  Set<CFGNode> result = new HashSet<>(getLeftOperand().nextNodesTT();
%  result.addAll(getLeftOperand().nextNodesFF());
%  return result;}
%
...
%eq IfStmt.getCondition().nextNodes() {
%  Set<CFGNode> result = new HashSet<>(getCondition().nextNodesTT());
%  result.addAll(getCondition().nextNodesFF());
%  return result;}
%
%eq PostUnaryDec.getOperand().nextNodes() = Collections.singleton(this);
%eq PostUnaryDec.firstNodes() = getOperand().firstNodes();
%\end{lstlisting}
%
%
%
